...
![DataAnalysis](https://starfire.site/upload/2017/11/1imo9fq5t6i7hodgt3pgi8jh4b.png)

从数据分析图中可以看出，关键节点的运行时间占据总运行时间的*30%-40%*。如果按照常见的排列方案，可以明显的发现当关键节点在运行时，其他PE几乎处于空闲状态，导致了极低的利用率。为了能够高效的利用在关键节点运行时其他PE空闲的时间， 本文中的算法利用重定时的技术把其他周期的节点安排在空闲时间内，并优先排列关键节点。下图展示了一个CNN网络图抽象建模成的有向无环图。

![图](./graph/graph.png)

![Baseline](./graph/Baseline.png)
